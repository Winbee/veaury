...
const createReactContainer = (Component, options, wrapInstance) =>
  class applyReact extends React.Component {
    // reactDevTools
    static displayName = `applyReact_${
      ...
      if (division) {
        if (children && children[0]) {
          return children.map((child, index) =>
            applyVueInReact(
              this.createSlot(child instanceof Function ? child : [child]),
              {
                ...options,
                ...customOptions,
                isSlots: true,
        ...
        Object.keys(Component.__syncUpdateForPureReactInVue).map(key => {
          if (finalProps[key] && typeof finalProps[key] === "function") {
            ...
            finalProps[key] = function (...args) {
              ...
              __veauryVueWrapperRef__.__veauryMountReactComponent__(
                true,
                true,
                {}
              );
...
export default function applyReactInVue(component, options = {}) {
  ...
  return {
    ...
    setup(props, context) {
      ...
      const injection =
        options.useInjectPropsFromWrapper ||
        component.__veauryInjectPropsFromWrapper__;
    ...
    methods: {
      ...
      __veauryPushVuePortal__(vuePortal) {
        const key =
          this.__veauryPortalKeyPool__.shift() ||
          this.__veauryMaxPortalCount__++;
      ...
      __veaurySyncUpdateProps__(extraData) {
        ...
        this.__veauryReactInstance__ &&
          this.__veauryReactInstance__.setState(extraData);
      },
      __veauryMountReactComponent__(update, updateType, extraData = {}) {
        ...
        if (!update || (update && updateType?.slot)) {
          ...
          for (let i in this.$slots || {}) {
            if (!this.$slots.hasOwnProperty(i) || this.$slots[i] == null)
              continue;
            const prefix = options.react.vueNamedSlotsKey.find(
              prefix => i.indexOf(prefix) === 0
            );
            if (prefix || i === "default") {
        ...
        if (!update) {
          ...
          if (reactWrapperRef) {
            ...
            this.reactPortal = () =>
              createPortal(reactRootComponent, container);
